...
const handleDates = (body: any): any => {
  ...
  Object.entries(body).forEach(([key, value]) => {
    if (isIso8601DateString(value)) {
      ...
      body[key] = format(parseISO(value as string), "YYYY-MM-DD");
...
export const transformDateToDateString = (request: InternalAxiosRequestConfig<any>): InternalAxiosRequestConfig<any> => {
